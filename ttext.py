...
def format_combined_response(primary_data, secondary_data=None):
    ...
    try:
        ...
        # Handle as raw data if not JSON
        formatted += f"📄 <b>Data:</b>\n<pre>{primary_data}</pre>\n"
        return formatted
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if primary_result or secondary_result:
            ...
            if secondary_result and len(str(secondary_result)) > len(str(primary_result or "")):
            ...
            # Check if the response contains actual data or just empty fields
            if formatted_response and has_meaningful_data(primary_result, secondary_result):
                # Deduct coins only if we have meaningful results
                user_credits[uid] -= 5
        ...
        await processing_msg.edit_text(
            "❌ Search failed due to technical error.\n"
