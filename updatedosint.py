...
OWNER_ID = 7835198116
API_URL = 'https://millionmack.com/mobile.php?key=GODMILLION&term='
...
async def fetch_phone_details(phone_number):
    ...
    try:
        ...
        logging.error(f"Error fetching phone details for {phone_number}: {e}")
    return None
...
async def fetch_aadhaar_details(aadhaar_number):
    ...
    try:
        ...
        logging.error(f"Error fetching Aadhaar details for {aadhaar_number}: {e}")
    return None
...
def format_combined_response(primary_data, secondary_data=None):
    ...
    try:
        ...
        # Handle as raw data if not JSON - but try to clean it up
        formatted += f"📄 <b>Raw Data:</b>\n"
        ...
        for line in clean_lines:
            if any(keyword in line.lower() for keyword in ['name:', 'phone:', 'address:', 'father:', 'circle:']):
                formatted += f"{line}\n"
    ...
    for record in all_records:
        if isinstance(record, dict):
            ...
            father = (record.get("father_name") or record.get("father") or "").lower().strip()
            
            # Use name+mobile+father as unique key to avoid exact duplicates
            signature = f"{name}_{mobile}_{father}"
            
            # Only keep if we haven't seen this exact combination
            if signature not in unique_records:
    ...
    record_count = 1
    all_linked_numbers = set()
    all_operators = set()
    
    for signature, record in unique_records.items():
        if record_count == 1:
            formatted += f"📄 <b>Data:</b>\n"
        else:
            formatted += f"📄 <b>Data {record_count}:</b>\n"
        
        # Name
        if record.get("name") and record.get("name").strip() not in ["N/A", "", "null"]:
            formatted += f"👤 <b>{record['name']}</b>\n"
        
        # Father Name
        father = record.get("father_name") or record.get("father") or record.get("guardian_name")
        if father and father.strip() not in ["N/A", "", "null"]:
            formatted += f"👨‍👦 {father}\n"
        
        # Primary Mobile
        if record.get("mobile") and record.get("mobile").strip():
            formatted += f"📱 <b>{record['mobile']}</b>\n"
            all_linked_numbers.add(record['mobile'])
        
        # Alternative number
        alt_num = record.get("alternative_number") or record.get("alt_number")
        if alt_num and alt_num.strip() not in ["N/A", "", "null"]:
            formatted += f"📞 Alt: {alt_num}\n"
            all_linked_numbers.add(alt_num)
        
        # Address (clean it up)
        if record.get("address") and record.get("address").strip() not in ["N/A", "", "null"]:
            address = record["address"].strip()
            # Clean up address formatting
            address_lines = [line.strip() for line in address.split('\n') if line.strip()]
            if address_lines:
                formatted += f"🏠 {', '.join(address_lines)}\n"
        
        # Circle/Operator
        if record.get("circle") and record.get("circle").strip():
            formatted += f"📍 {record['circle']}\n"
            all_operators.add(record['circle'])
        
        # Aadhaar
        aadhaar = (record.get("aadhaar") or record.get("id_number") or 
                   record.get("aadhar") or record.get("aadhaar_number"))
        if aadhaar and str(aadhaar).strip() not in ["N/A", "", "null", "0"]:
            formatted += f"🆔 {aadhaar}\n"
        
        # Email
        if record.get("email") and record.get("email").strip() not in ["N/A", "", "null"]:
            formatted += f"📧 {record['email']}\n"
        
        formatted += "\n"
        record_count += 1

    # Show summary if multiple numbers found
    if len(all_linked_numbers) > 1:
        formatted += f"🔗 <b>All Linked Numbers:</b>\n"
        for number in sorted(all_linked_numbers):
            formatted += f"• {number}\n"
        formatted += "\n"
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if is_mobile_number(cleaned_que